...
class Levels:
    ...
    def get_persistence(self, graph=False):
        ...
        dig_up = np.asarray(
            [[ele[1][0], ele[1][1]] for ele in dig_up if ele[1][1] < np.inf]
        )
        dig_dw = np.asarray(
            [[ele[1][0], ele[1][1]] for ele in dig_dw if ele[1][1] < np.inf]
        )
    ...
    # graph refers whether to display a graph or not
    def betti_curves(
        self, mnu=None, mxu=None, mnd=None, mxd=None, num_points=100, graph=False
    ):
